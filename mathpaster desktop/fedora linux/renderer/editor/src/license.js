 ...
 * No backend: the desktop app talks directly to Lemon Squeezy's public license
 * API (activate/validate/deactivate need no API key and allow CORS from
 * anywhere). The license state is cached in localStorage. Validation is re-run
 * silently once a week and FAILS OPEN — a
 * paying user never loses Pro because they're offline.
 */
export const CHECKOUT_URL = 'https://mathpaster.lemonsqueezy.com/checkout/buy/c4b9e824-6289-4913-b467-028670ec5c7f';
...
// can try every Pro feature. The desktop app stays gated behind a real key.
const IS_WEB_DEMO = new URLSearchParams(location.search).has('demo');
 ...
 * Programmatic tab-opening (window.open / synthetic anchor clicks)
